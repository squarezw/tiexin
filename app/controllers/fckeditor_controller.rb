...
class FckeditorController < ApplicationController
  ...
  def quick_upload
    params[:Type] = "Image" # only to pic path
    begin
      @new_file = check_file(params[:NewFile])
      @fck_url = upload_directory_path  
      ftype = @new_file.content_type.strip
      if ! MIME_TYPES.include?(ftype)
        ...
        @web_path = "#{UPLOADED}/#{params[:Type]}/#{Time.now.year}/#{Time.now.month}/#{Time.now.to_i}_#{@new_file.original_filename}"         
        path = date_directory_path  + "/#{Time.now.to_i}_" + @new_file.original_filename # 按时间生成
        #path = current_directory_path + "/" + @new_file.original_filename # 原始版本的方法
        File.open(path,"wb",0664) do |fp|
          FileUtils.copy_stream(@new_file, fp)
        end        
        @errorNumber = 0
    ...
    if @errorNumber == 0
        responds_to_parent do
           render :update do |page|
              page.call 'insert_img', "#{@web_path}", @new_file.original_filename
           end
        end
    else
      err_msg =err_msg(@errorNumber)
      responds_to_parent do
        render :update do |page|
          page.alert err_msg
        end
