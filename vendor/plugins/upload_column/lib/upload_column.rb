...
module UploadColumn
  ...
  DEFAULT_OPTIONS = {
    ...
    :store_dir => proc{|inst, attr| File.join(ActiveSupport::Inflector.underscore(inst.class).to_s, attr.to_s, inst.id.to_s) },
    :tmp_dir => proc{|inst, attr| File.join(ActiveSupport::Inflector.underscore(inst.class).to_s, attr.to_s, "tmp") },
    :old_files => :delete,
  ...
  class UploadedFile
    ...
    def assign(file, directory = nil )
      ...
      if temp_path = ( file.respond_to?(:local_path) ? file.local_path : file.path ) and temp_path != "" 
        ...
        self.filename = fetch_filename( self.instance, basename, self.ext )
        puts "filename_extension: #{self.filename_extension}"
        return false unless check_integrity( self.filename_extension )
      ...
      puts "chmod #{options[:permissions]} #{self.path}"
  ...
  module ClassMethods
    ...
    def validates_integrity_of(*attr_names)
      ...
      validates_each(attr_names, configuration) do |record, attr, column|    
        # Modified by Gu Xiaofeng. downcase file name extension before validation.
#        logger.info column.options[:extensions].to_a.join(',')
#        logger.info column.filename_extension.downcase
        if column and not column.options[:extensions].include?( column.filename_extension.downcase )
